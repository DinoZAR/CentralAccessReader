...
function HighlightNextWord(doLine, word, wordOffset, wordLength) {
	...
	while (true) {
		...
		console.debug(' -------- HighlightWord: Checking node: ' + elem.toString() + ' for word <' + word + '>');
		if (elem.nodeType === Node.TEXT_NODE) {
			console.debug(' -------- HighlightWord: ' + elem.data);
		}
			...
			if (elem.nodeType === Node.TEXT_NODE) {
				...
				if ((wordOffset + wordLength) <= elem.data.length) {
					...
					if (elem.data.indexOf(word) >= 0) {
						...
						//console.debug('HighlightWord: Failed inside equation check');
					}
					//console.debug('HighlightWord: Failed word present check');
				}
				//console.debug('HighlightWord: Failed bound check');
			}
			//console.debug('HighlightWord: Failed node type check');
		}
		//console.debug('HighlightWord: Failed word offset');
		console.debug('HighlightWord: Trying next element...');
		startOffset = 0;
		needScroll = true;
	...
	console.debug('HighlightWord: Out of the check loop...');
	
	// If I actually got an element, set the highlight
	if ((elem !== null) && (isHighlighting === true)) {
		console.debug('HighlightWord: Got element to highlight...');
		var r = document.createRange();
		r.setStart(elem, wordOffset);
		r.setEnd(elem, wordOffset + wordLength);
	...
	console.debug('___________________________________________________________');
}
...
function HighlightNextImage(doLine) {
	...
	console.debug('Start element for image: ' + elem.toString());
	if (elem.nodeType === Node.TEXT_NODE) {
		console.debug('Text content: ' + elem.data);
	}
	...
	if ((elem !== null) && (isHighlighting === true)) {
		...
		console.debug('My highlight range: ' + GetHTMLSource(r));
...
function GetReferencePoint() {
	...
	if (NoHighlights()) {
		...
		console.debug('------ Result index: ' + results.index.toString());
		console.debug('------ Result offset: ' + results.offset.toString());
		console.debug('------ Result element: ' + myElem.toString());
