...
from car.gui.bookmarks import BookmarkNode
...
#         self._mathParserThread.start()
    ...
    def _prepareHead(self, head, mathOutput):
        ...
        mathjaxConfigFile = ''
        if mathOutput == 'svg':
            mathjaxConfigFile = misc.program_path('car/javascript/mathjax_config_svg.jsconfig')
        else:
            mathjaxConfigFile = misc.program_path('car/javascript/mathjax_config.jsconfig')
        ...
        javascriptFiles = [f for f in os.listdir(misc.program_path('car/javascript/'))
                           if os.path.isfile(os.path.join(misc.program_path('car/javascript/'), f)) and
                           os.path.splitext(os.path.join(misc.program_path('car/javascript/'), f))[1] == '.js']
        javascriptElements = []
        for f in javascriptFiles:
            elem = HTML.Element('script')
            elem.set('language', 'javascript')
            elem.set('type', 'text/javascript')
            with open(os.path.join(misc.program_path('car/javascript/'), f), 'r') as jsFile:
