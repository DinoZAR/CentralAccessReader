...
from car.speech.drivers.SapiCOM import SpVoice, SpFileStream, constants
...
class SAPI5Driver(object):
    ...
    def speakToFile(self, mp3Path, speechGenerator, tempDirectory, progressCallback, labelCallback, checkStopFunction):
        ...
        wavPath = os.path.join(tempDirectory, 'tmp.wav')
        
        # Try to remove it, if one exists already
        try:
            os.remove(wavPath)
        except OSError:
            pass
        ...
        if not checkStopFunction():
            ...
            if '64' in platform.architecture()[0]:
                lameExe = program_path('car/lame_64.exe')
            else:
                lameExe = program_path('car/lame_32.exe')
             
            startupInfo = subprocess.STARTUPINFO()
            startupInfo.dwFlags = subprocess.STARTF_USESTDHANDLES | subprocess.STARTF_USESHOWWINDOW
            startupInfo.wShowWindow = subprocess.SW_HIDE
            lameCommand = lameExe + ' -h "' + wavPath + '" "' + mp3Path + '"'
